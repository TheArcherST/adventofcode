...
class Solution(AdventOfCodePuzzle):
    def task_1(self, data):
        ...
        def is_signal(number: int):
            return (number - 20) % 40 == 0
        calln = 0
        result = 0

        def nextc():
            nonlocal calln, result, sum_
            calln+=1
            
            if is_signal(calln):
                self.console.print(sum_, calln)
    ...
    def task_2(self, data):
        ...
        def nextc():
            ...
            current_crt = ((calln-1) // 40)
            tilemap = '.'*(x-1) + '#'*3 + '.'*(40-3-(x-1))
            crt[current_crt] += tilemap[calln-1-current_crt*40] + ''
